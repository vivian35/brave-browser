...
const test = (suite, buildConfig = config.defaultBuildConfig, options) => {
  ...
  if (run_brave_rewards_unitests) {
    util.run('ninja', ['-C', config.outputDir, 'brave_rewards_unit_tests'], config.defaultOptions)
  }
  ...
  if (config.targetOS === 'ios') {
    ...
    let testBinary;
    let rewardsTestBinary;
    let installerTestBinary;
    if (process.platform === 'win32') {
      testBinary = `${suite}.exe`
      rewardsTestBinary = 'brave_rewards_unit_tests.exe'
      installerTestBinary = 'brave_installer_unittests.exe'
    } else {
      testBinary = suite
      rewardsTestBinary = 'brave_rewards_unit_tests'
    ...
    if (run_brave_rewards_unitests) {
      ...
      if (options.output) {
        ...
        braveArgs.push('--gtest_output=xml:brave_rewards_unit_tests.xml')
      }

      util.run(path.join(config.outputDir, rewardsTestBinary), braveArgs, config.defaultOptions)
    }
